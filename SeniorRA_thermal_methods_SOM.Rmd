...
bibliography: [refs.bib, refs_som.bib]
header-includes:
- \usepackage{natbib}
- \usepackage{fontspec}
...
Rebecca A. Senior^1^*, Jane K. Hill^2^ and David P. Edwards^1^
...
(ref:cap-B-2) Schematic summarising the key functions for processing groups of images (left) or a single image (right).

```{r fig-B-2, fig.cap= "(ref:cap-B-2)", fig.pos = 'H'}
knitr::include_graphics("../figures/schematic/schematic.png")
```
...
In addition to patch statistics, `get_stats` can return (1) the temperature dataset in a dataframe format, and (2) a `SpatialPolygonsDataFrame` of its hot and cold spots. The function `plot_patches` can then recreate the original thermal image overlaid with outlines of hot and cold spots, as well as the temperature distribution if `plot_distribution = TRUE` (\autoref{fig:fig-B-1}).

(ref:cap-B-1) The output of `plot_patches` includes a histogram and the original temperature data overlaid with outlines of hot and cold spots, identified using the G\* variant of the Getis-Ord local statistic.
